...
void ParseTabFile(factor::morpho::aot::AotParser::Ptr parser, const std::string& fname, const std::string& model, Tabs& tabs) {
  // Open tab file.
...
void ReadSuffixSection(factor::morpho::aot::AotParser::Ptr parser, std::ifstream& mfile, m::Morphologist<Alphabet>& morpher, const Tabs& tabs) {
  ...
  for (unsigned i = 0; i < line_count; ++i) {
    ...
    for (unsigned i = 1; i < fields.size(); ++i) { // Drop first field.
      ...
      if (it == tabs.end()) {
        ...
        typedef factor::iterators::Utf8Iterator<std::string::iterator> Utf8Iterator;
        std::vector<uint32_t> norm_suffix;
        for (Utf8Iterator sym_it(pair[0].begin(), pair[0].end()); sym_it != Utf8Iterator(); ++sym_it) {
          norm_suffix.push_back(factor::symbols::ToLower(*sym_it));
        }
         // Add suffix.
        m::MorphoModifier::AddSuffix(morpher, line_id, it->second, norm_suffix.begin(), norm_suffix.end());

        // Remember main form suffix for this line.
        if (parser->IsMainFormCode(pair[1])) {
          std::string suffix;
          factor::symbols::GetUtf8Sequence(norm_suffix.begin(), norm_suffix.end(), std::back_inserter(suffix));
...
template<class Alphabet>
void ReadDictionarySection(std::ifstream& mfile, m::Morphologist<Alphabet>& morpher) {
  ...
  for (unsigned i = 0; i < line_count; ++i) {
    ...
    for (Utf8Iterator sym_it(fields[0].begin(), fields[0].end()); sym_it != Utf8Iterator(); ++sym_it) {
      norm_base.push_back(factor::symbols::ToLower(*sym_it));
    }

    // Calculate main form.
    std::string main_form;
    factor::symbols::GetUtf8Sequence(norm_base.begin(), norm_base.end(), std::back_inserter(main_form));
    main_form += main_form_suffix_list[line_id];

    m::MorphoModifier::AddBase(morpher, lem_id, line_id, norm_base.begin(), norm_base.end(), main_form);
  }
...
template<class Alphabet>
void ReadDictFile(factor::morpho::aot::AotParser::Ptr parser, const std::string& dname, const std::string& bname, const Tabs& tabs) {
  std::cerr << "Starting parse dictionary file...\n";
  m::Morphologist<Alphabet> morpher;
...
int main(int argc, char* argv[]) {
  try {
    ...
    factor::morpho::aot::AotParser::Ptr parser;
    if (model == "rus") {
      parser = factor::morpho::aot::AotParser::Ptr(new factor::morpho::aot::RussianAotParser());
    } else if (model == "eng") {
      parser = factor::morpho::aot::AotParser::Ptr(new factor::morpho::aot::EnglishAotParser());
    } else {
    ...
    if (model == "rus") {
      ReadDictFile<m::RussianAlphabet>(parser, dict_file_name, bin_file_name, tabs);
    } else if (model == "eng") {
      ReadDictFile<m::EnglishAlphabet>(parser, dict_file_name, bin_file_name, tabs);
    }
